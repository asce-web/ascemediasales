...
class FilebrowserStorage {
  ...
  public function storageLoadMultipleData($nids, $access = true) {
    $query = $this->connection->select('filebrowser_nodes', 'fb', array('fetch' => \PDO::FETCH_ASSOC));
    $query->fields('fb');
  ...
  public function storageLoadData($nid) {
    $query = $this->connection->select('filebrowser_nodes', 'f', array('fetch' => \PDO::FETCH_ASSOC));
    $query->fields('f');
